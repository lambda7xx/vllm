...
def with_agent(engine,args, prompts):
    ...
    while True:
        ...
        while reqs:
            ...
            if req_num_act % 2 == 1:
                engine.add_request(request_id = rid1, prompt = sp1+ prompt, params = sampling_params, arrival_time = now)
                #do prefill for req2 
                engine.add_request(request_id = rid2, prompt = sp2+ prompt, params = discard_sampling_params, arrival_time = now)
            else:
                engine.add_request(request_id = rid2, prompt = sp2+ prompt, params = sampling_params, arrival_time = now)
                #do prefill for req1
                engine.add_request(request_id = rid1, prompt = sp1+ prompt, params = discard_sampling_params, arrival_time = now)
            try:
